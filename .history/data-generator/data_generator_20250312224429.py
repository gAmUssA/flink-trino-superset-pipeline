...
def generate_sensor_data():
    ...
    if sensor_type == 'temperature':
        data['value'] = round(random.uniform(-10, 40), 1)  # Celsius
        data['unit'] = 'C'
    elif sensor_type == 'humidity':
        data['value'] = round(random.uniform(0, 100), 1)  # Percentage
        data['unit'] = '%'
    elif sensor_type == 'pressure':
        data['value'] = round(random.uniform(970, 1030), 1)  # hPa
        data['unit'] = 'hPa'
    elif sensor_type == 'light':
        data['value'] = round(random.uniform(0, 1000), 1)  # Lux
        data['unit'] = 'lux'
    elif sensor_type == 'motion':
        data['value'] = random.choice([0, 1])  # Binary (detected or not)
        data['unit'] = 'binary'
